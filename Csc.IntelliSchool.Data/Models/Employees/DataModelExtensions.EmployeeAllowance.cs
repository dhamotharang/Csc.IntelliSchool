...
namespace Csc.IntelliSchool.Data {
  public static partial class DataModelExtensions {
    ...
    public static IQueryable<EmployeeAllowance> Query(this DbQuery<EmployeeAllowance> dbQry) {
      return Query(dbQry, EmployeeAllowanceIncludes.None, null);
    }

    public static IQueryable<EmployeeAllowance> Query(this DbQuery<EmployeeAllowance> dbQry, EmployeeAllowanceIncludes filter) {
      return Query(dbQry, filter, null);
    }

    public static IQueryable<EmployeeAllowance> Query(this DbQuery<EmployeeAllowance> dbQry, EmployeeDataCriteria crit) {
      return Query(dbQry, EmployeeAllowanceIncludes.None, crit);
    }

    public static IQueryable<EmployeeAllowance> Query(this DbQuery<EmployeeAllowance> dbQry,
      EmployeeAllowanceIncludes filter,
      EmployeeDataCriteria crit) {

      IQueryable<EmployeeAllowance> qry = null;

      dbQry = dbQry.Include(filter);

      if (crit != null) {
        if (filter.HasFlag(EmployeeAllowanceIncludes.Employee))
        ...
        if (crit.ItemIDs != null && crit.ItemIDs.Count() > 0) {
          qry = qry.Where(s => crit.ItemIDs.Contains(s.AllowanceID));
        }
