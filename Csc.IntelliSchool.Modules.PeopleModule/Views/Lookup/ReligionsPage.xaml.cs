...
namespace Csc.IntelliSchool.Modules.PeopleModule.Views.Lookup {
  public partial class ReligionsPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<Religion> _items;
    #endregion


    #region Properties
    public ObservableCollection<Religion> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion
    ...
    private void OnLoadData() {
      this.SetBusy();
      PeopleDataManager.GetReligions(true, OnDataLoaded);
    }
    private void OnDataLoaded(Religion[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<Religion>(result);
      else
    ...
    private void OnEditItem() {
      ReligionModifyWindow wnd = new ReligionModifyWindow((Religion)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.ConfirmDelete(null, () => {
        this.SetBusy();
        PeopleDataManager.DeleteReligion((Religion)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((ReligionModifyWindow)sender);
