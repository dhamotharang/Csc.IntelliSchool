...
namespace Csc.IntelliSchool.Modules.HRModule.Views {
  public partial class ChargesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeCharge> _items;
    private EmployeeBranch[] _branches;
    private EmployeeDepartment[] _departments;
    private EmployeePosition[] _positions;
    private DateTime? _selectedStartDate;
    private DateTime? _selectedEndDate;
    private EmployeeChargeType[] _types;
    ...
    public ObservableCollection<EmployeeCharge> AllItems { get; set; }
    public ObservableCollection<EmployeeCharge> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    ...
    public EmployeeChargeType[] Types { get { return _types; } set { if (_types != value) { _types = value; OnPropertyChanged(() => Types); } } }
    ...
    public int[] SelectedTypeIDs {
      get {
        return this.TypeComboBox.GetSelectedItemIDs<EmployeeChargeType>(a => a.TypeID);
      }
      set {
        this.TypeComboBox.SetSelectedItemIDs<EmployeeChargeType>(value ?? new int[] { }, a => a.TypeID);
      }
    }
    ...
    private void OnGetChargeTypes() {
      this.SetBusy();
      EmployeesDataManager.GetChargeTypes(false, OnGetTypes);
    }
    ...
    private void OnGetTypes(EmployeeChargeType[] result, Exception error) {
      if (error == null) {
    ...
    private void OnLoadData(bool reload = false) {
      ...
      if (SelectedTypeIDs.Count() == 0) {
        OnDataLoaded(new EmployeeCharge[] { }, null);
        return;
      }

      this.SetBusy();
      EmployeesDataManager.GetCharges(SelectedTypeIDs,
        SelectedStartDate.Value, SelectedEndDate.Value,
        null, ListID != null ? ListID.Value.PackArray() : null, true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeCharge[] result, Exception error) {
      ...
      if (error == null) {
        ...
        AllItems = new ObservableCollection<EmployeeCharge>(result);
        OnFilterData();
      } else
        Popup.AlertError(error);
    ...
    private void OnFilterData(object itm = null) {
      ...
      if (AllItems.Count() > 0 && lists.Count() > 0) {
        ...
        Items = new ObservableCollection<EmployeeCharge>(qry.ToArray());
      } else
        Items = new ObservableCollection<EmployeeCharge>();
    ...
    private void OnDeleteItem() {
      var itm = (EmployeeCharge)this.ItemsGridView.SelectedItem;
      Popup.ConfirmDelete(null, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteCharge(itm, OnDeleted);
      });
    }
    ...
    private void OnEmployeeSelected(EmployeeSelectionWindow obj) {
      ...
      var wnd = new ChargeModifyWindow(obj.SelectedEmployee);
      this.DisplayModal<ChargeModifyWindow>(wnd, OnModifyWindowClosed);
    }

    private void OnModifyWindowClosed(ChargeModifyWindow obj) {
      if (obj.Result == OperationResult.None)
        return;

      var itemStartDate = obj.Item.StartMonth ?? DateTime.MinValue;
      var itemEndDate = obj.Item.EndMonth ?? DateTime.MaxValue;

      bool validItem = DateTimeExtensions.Overlaps(itemStartDate, itemEndDate, CurrentStartDate , CurrentEndDate ) &&
    ...
    private void OnEditItem() {
      var itm = (EmployeeCharge)this.ItemsGridView.SelectedItem;
      ChargeModifyWindow wnd = new ChargeModifyWindow(itm.Employee, itm);
      this.DisplayModal<ChargeModifyWindow>(wnd, OnModifyWindowClosed);
    }
    #endregion
