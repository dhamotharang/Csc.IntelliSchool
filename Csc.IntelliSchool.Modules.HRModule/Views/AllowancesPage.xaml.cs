...
namespace Csc.IntelliSchool.Modules.HRModule.Views {
  public partial class AllowancesPage : Csc.Wpf.PageBase {
    #region Fields
    ...
    private EmployeeAllowanceType[] _types;
    ...
    public ObservableCollection<EmployeeAllowance> AllItems { get; set; }
    ...
    public EmployeeAllowanceType[] Types { get { return _types; } set { if (_types != value) { _types = value; OnPropertyChanged(() => Types); } } }
    ...
    public int[] SelectedTypeIDs {
      get {
        return this.TypeComboBox.GetSelectedItemIDs<EmployeeAllowanceType>(a => a.TypeID);
      }
      set {
        this.TypeComboBox.SetSelectedItemIDs<EmployeeAllowanceType>(value ?? new int[] { }, a => a.TypeID);
      }
    }
    ...
    private void OnGetAllowanceTypes() {
      this.SetBusy();
      EmployeesDataManager.GetAllowanceTypes(false, OnGetTypes);
    }
    ...
    private void OnGetTypes(EmployeeAllowanceType[] result, Exception error) {
      if (error == null) {
    ...
    private void OnLoadData(bool reload = false) {
      ...
      if (SelectedTypeIDs.Count() == 0) {
        OnDataLoaded(new EmployeeAllowance[] { }, null);
        return;
      }

      this.SetBusy();
      EmployeesDataManager.GetAllowances(SelectedTypeIDs,
    ...
    private void OnDataLoaded(EmployeeAllowance[] result, Exception error) {
      ...
      if (error == null) {
        ...
        AllItems = new ObservableCollection<EmployeeAllowance>(result);
        OnFilterData();
      } else
        Popup.AlertError(error);
    ...
    private void OnFilterData(object itm = null) {
      ...
      if (AllItems.Count() > 0 && lists.Count() > 0) {
        ...
        Items = new ObservableCollection<EmployeeAllowance>(qry.ToArray());
      } else
    ...
    private void OnEmployeeSelected(EmployeeSelectionWindow obj) {
      ...
      var wnd = new AllowanceModifyWindow(obj.SelectedEmployee);
      this.DisplayModal<AllowanceModifyWindow>(wnd, OnModifyWindowClosed);
    }

    private void OnModifyWindowClosed(AllowanceModifyWindow obj) {
    ...
    private void OnEditItem() {
      var itm = (EmployeeAllowance)this.ItemsGridView.SelectedItem;
      AllowanceModifyWindow wnd = new AllowanceModifyWindow(itm.Employee, itm);
      this.DisplayModal<AllowanceModifyWindow>(wnd, OnModifyWindowClosed);
    }
    #endregion
