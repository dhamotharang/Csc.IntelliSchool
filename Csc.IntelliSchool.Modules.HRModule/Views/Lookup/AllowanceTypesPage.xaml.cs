...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class AllowanceTypesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeAllowanceType> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeAllowanceType> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion


    // Constructors
    public AllowanceTypesPage() {
      InitializeComponent();
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetAllowanceTypes(true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeAllowanceType[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeAllowanceType>(result);
      else
    ...
    private void OnEditItem() {
      AllowanceTypeModifyWindow wnd = new AllowanceTypeModifyWindow((EmployeeAllowanceType)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.Confirm(null, Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteAllowanceType((EmployeeAllowanceType)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }


    private void OnDeleted(EmployeeAllowanceType result, Exception error) {
      if (error == null)
        Items.Remove(result);
      else
        Popup.AlertError(error);
    ...
    private void OnNewItem() {
      AllowanceTypeModifyWindow wnd = new AllowanceTypeModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((AllowanceTypeModifyWindow)sender);
