...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class ShiftOverrideTypesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeShiftOverrideType> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeShiftOverrideType> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion


    // Constructors
    public ShiftOverrideTypesPage() {
      InitializeComponent();
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetShiftOverrideTypes( OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeShiftOverrideType[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeShiftOverrideType>(result);
      else
    ...
    private void OnEditItem() {
      ShiftOverrideTypeModifyWindow wnd = new ShiftOverrideTypeModifyWindow((EmployeeShiftOverrideType)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.Confirm(null, Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteShiftOverrideType((EmployeeShiftOverrideType)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }


    private void OnDeleted(EmployeeShiftOverrideType result, Exception error) {
      if (error == null)
        Items.Remove(result);
      else
        Popup.AlertError(error);
    ...
    private void OnNewItem() {
      ShiftOverrideTypeModifyWindow wnd = new ShiftOverrideTypeModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((ShiftOverrideTypeModifyWindow)sender);
