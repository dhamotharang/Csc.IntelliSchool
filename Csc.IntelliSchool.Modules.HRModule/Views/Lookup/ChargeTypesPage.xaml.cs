...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class ChargeTypesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeChargeType> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeChargeType> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion


    // Constructors
    public ChargeTypesPage() {
      InitializeComponent();
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetChargeTypes(true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeChargeType[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeChargeType>(result);
      else
    ...
    private void OnEditItem() {
      ChargeTypeModifyWindow wnd = new ChargeTypeModifyWindow((EmployeeChargeType)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.Confirm(null, Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteChargeType((EmployeeChargeType)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }


    private void OnDeleted(EmployeeChargeType result, Exception error) {
      if (error == null)
        Items.Remove(result);
      else
        Popup.AlertError(error);
    ...
    private void OnNewItem() {
      ChargeTypeModifyWindow wnd = new ChargeTypeModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((ChargeTypeModifyWindow)sender);
