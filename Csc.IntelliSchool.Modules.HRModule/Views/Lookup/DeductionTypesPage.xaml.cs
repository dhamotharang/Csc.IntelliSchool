...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class DeductionTypesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeDeductionType> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeDeductionType> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion


    // Constructors
    public DeductionTypesPage() {
      InitializeComponent();
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetDeductionTypes(true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeDeductionType[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeDeductionType>(result);
      else
    ...
    private void OnEditItem() {
      DeductionTypeModifyWindow wnd = new DeductionTypeModifyWindow((EmployeeDeductionType)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.Confirm(null, Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteDeductionType((EmployeeDeductionType)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }


    private void OnDeleted(EmployeeDeductionType result, Exception error) {
      if (error == null)
        Items.Remove(result);
      else
        Popup.AlertError(error);
    ...
    private void OnNewItem() {
      DeductionTypeModifyWindow wnd = new DeductionTypeModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((DeductionTypeModifyWindow)sender);
