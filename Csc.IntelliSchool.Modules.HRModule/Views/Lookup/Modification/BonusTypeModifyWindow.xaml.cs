...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class BonusTypeModifyWindow : Csc.Wpf.WindowBase {
    // Fields
    private EmployeeBonusType _item;

    // Properties
    public EmployeeBonusType Item { get { return _item; } protected set { _item = value; OnPropertyChanged(() => Item); } }
    public EmployeeBonusType OriginalItem { get; set; }

    // Constructors
    public BonusTypeModifyWindow() {
      InitializeComponent();
      Item = new EmployeeBonusType();
      
    }
    public BonusTypeModifyWindow(EmployeeBonusType item)
      : this() {
        if (item != null) {
          OriginalItem = item;
          Item = item.Clone();
    ...
    private void DeleteButton_Click(object sender, RoutedEventArgs e) {
      this.Confirm(Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType,  () => {
        this.SetBusy();
        EmployeesDataManager.DeleteBonusType(Item, OnDeleteCompleted);
      });
    }



    private void OnDeleteCompleted(EmployeeBonusType result, Exception error) {
      if (error == null) {
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e) {
      ...
      this.SetBusy();
      if (Item.TypeID == 0)
        EmployeesDataManager.AddBonusType(Item, OnAdded);
      else {
        EmployeesDataManager.UpdateBonusType(Item, OnUpdated);
      }
    }

    private void OnAdded(EmployeeBonusType result, Exception error) {
      if (error == null) {
        Item = result;
    ...
    private void OnUpdated(EmployeeBonusType result, Exception error) {
      if (error == null) {
        Item = result;
