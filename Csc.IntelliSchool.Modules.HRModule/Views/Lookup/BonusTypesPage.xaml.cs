...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Lookup {
  public partial class BonusTypesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeBonusType> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeBonusType> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion


    // Constructors
    public BonusTypesPage() {
      InitializeComponent();
    }
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetBonusTypes(true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeBonusType[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeBonusType>(result);
      else
    ...
    private void OnEditItem() {
      BonusTypeModifyWindow wnd = new BonusTypeModifyWindow((EmployeeBonusType)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.Confirm(null, Csc.IntelliSchool.Assets.Resources.HumanResources.Confirm_DeleteType, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteBonusType((EmployeeBonusType)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }


    private void OnDeleted(EmployeeBonusType result, Exception error) {
      if (error == null)
        Items.Remove(result);
      else
        Popup.AlertError(error);
    ...
    private void OnNewItem() {
      BonusTypeModifyWindow wnd = new BonusTypeModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((BonusTypeModifyWindow)sender);
