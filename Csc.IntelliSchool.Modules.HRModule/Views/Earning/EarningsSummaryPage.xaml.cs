...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Earning {
  public partial class EarningsSummaryPage : Csc.Wpf.PageBase {
    #region Fields
    private EmployeeBranch[] _branches;
    private EmployeeDepartment[] _departments;
    private ObservableCollection<EmployeeEarningSummary> _items;
    private EmployeePosition[] _positions;
    #endregion

    #region Properties
    public ObservableCollection<EmployeeEarningSummary> AllItems { get; set; }
    ...
    public ObservableCollection<EmployeeEarningSummary> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    ...
    private void OnDataLoaded(EmployeeEarningSummary[] result, Exception error) {
      if (error == null) {
        CurrentStartMonth = SelectedStartMonth;
        CurrentEndMonth = SelectedEndMonth;

        AllItems = new ObservableCollection<EmployeeEarningSummary>(result.OrderBy(s => s.Employee.Person.FullName));
        OnFilterData();
      } else
    ...
    private void OnFilterData() {
      ...
      if (AllItems.Count() > 0 && lists.Count() > 0) {
        ...
        Items = new ObservableCollection<EmployeeEarningSummary>(qry.ToArray());
      } else {
        Items = new ObservableCollection<EmployeeEarningSummary>();
      }
    ...
    private void OnLoadData() {
      ...
      if (this.SelectedStartMonth == null || this.SelectedEndMonth == null) {
        OnDataLoaded(new EmployeeEarningSummary[] { }, null);
        return;
      }

      this.SetBusy();
      EmployeesDataManager.GetEarningsSummary(SelectedStartMonth.Value, SelectedEndMonth.Value, ListID != null ? ListID.Value.PackArray() : null, OnDataLoaded);
    }
    ...
    private void OnEarning() {
      var earn = ((EmployeeEarningSummary)this.ItemsGridView.SelectedItem);
      var wnd = new Earning.EarningWindow(earn.Employee);
      this.DisplayModal < EarningWindow>(wnd, earn);
    }
    #endregion
    ...
    private void PrintEarningsMenuItem_Click(object sender, Telerik.Windows.RadRoutedEventArgs e) {
    ...
    private void OnPrintEarnings(EmployeeFilterWindow obj) {
      ...
      var items = obj.ItemsAs<EmployeeEarningSummary>().ToArray();
       this.DisplayReport(new Reports.EarningsSummaryReport() {
    ...
    private void PrinAttendanceMenuItem_Click(object sender, Telerik.Windows.RadRoutedEventArgs e) {
      if (AllItems == null || AllItems.Count() == 0 || this.CurrentStartMonth == null || this.CurrentEndMonth == null)
        return;
    ...
    private void OnPrintAttndance(EmployeeFilterWindow obj) {
      ...
      this.DisplayReport(new Reports.AttendanceSummaryReport() {
        StartMonth = this.CurrentStartMonth.Value,
        EndMonth = this.CurrentEndMonth.Value
      }, items.Select(s => (EmployeeEarningSummary)s)
         .GroupBy(s => s.Employee.Department).OrderBy(s => s.Key != null ? s.Key.Name : null).ToArray(), null);
    }
  }
}
