...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Earning {
  public partial class EarningVacationsControl : Csc.Wpf.UserControlBase, IEarningControl {
    ...
    public override bool HasUpdates { get { return _hasUpdates;} }
    public EmployeeEarningSection Section { get { return EmployeeEarningSection.Vacations; } }

    public EarningVacationsControl() {
      InitializeComponent();
    ...
    public void OnLoadData(bool force = false) {
      ...
      this.SetBusy();
      EmployeesDataManager.GetVacations(Employee.EmployeeID, month, filter, OnDataLoaded);
    }
    ...
    private void OnDataLoaded(EmployeeVacation[] result, Exception error) {
      ...
      if (error == null) {
        ...
        Items = new ObservableCollection<EmployeeVacation>(result.OrderBy(s => s.StartDate).ToArray());
        //if (sort)
    ...
    private void OnNewItem() {
      VacationModifyWindow wnd = new VacationModifyWindow(Employee);
      this.DisplayModal<VacationModifyWindow>(wnd, OnModifyClosed);
    }

    private void OnModifyClosed(VacationModifyWindow wnd) {
      if (wnd.DialogResult != true)
        return;

      if (wnd.Result == OperationResult.Add && DateTimeExtensions.Overlaps(this.LastSelectedMonth.Value, LastSelectedFilter.Value, wnd.Item.StartDate, wnd.Item.EndDate)) {
        Items.Add(wnd.Item);
        ...
        Items.Remove(wnd.OriginalItem);
        if (DateTimeExtensions.Overlaps(this.LastSelectedMonth.Value, LastSelectedFilter.Value, wnd.Item.StartDate, wnd.Item.EndDate)) {
          Items.Add(wnd.Item);
          this.ItemsGridView.SelectItem(wnd.Item);
        }
      };
    ...
    private void OnEditItem() {
      VacationModifyWindow wnd = new VacationModifyWindow(Employee, (EmployeeVacation)this.ItemsGridView.SelectedItem);
      this.DisplayModal<VacationModifyWindow>(wnd, OnModifyClosed);
    }
