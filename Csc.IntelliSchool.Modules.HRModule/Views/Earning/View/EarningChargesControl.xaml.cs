...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Earning {
  public partial class EarningChargesControl : Csc.Wpf.UserControlBase, IEarningControl {
    ...
    public EmployeeEarningSection Section { get { return EmployeeEarningSection.Charges; } }


    public EarningChargesControl() {
      InitializeComponent();
    ...
    public void OnLoadData(bool force = false) {
      ...
      this.SetBusy();
      EmployeesDataManager.GetCharges(Employee.EmployeeID, month, filter, OnDataLoaded);
    }
    ...
    private void OnDataLoaded(EmployeeCharge[] result, Exception error) {
      ...
      if (error == null) {
        ...
        Items = new ObservableCollection<EmployeeCharge>(result.OrderBy(s => s.StartMonth).ToArray());
        //if (sort)
    ...
    private void OnNewItem() {
      ChargeModifyWindow wnd = new ChargeModifyWindow(Employee);
      this.DisplayModal<ChargeModifyWindow>(wnd, OnModifyClosed);
    }

    private void OnModifyClosed(ChargeModifyWindow wnd) {
    ...
    private void OnEditItem() {
      ChargeModifyWindow wnd = new ChargeModifyWindow(Employee, (EmployeeCharge)this.ItemsGridView.SelectedItem);
      this.DisplayModal<ChargeModifyWindow>(wnd, OnModifyClosed);
    }
