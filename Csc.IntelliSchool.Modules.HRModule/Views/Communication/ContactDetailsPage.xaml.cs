...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Communication {
  public partial class ContactDetailsPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeContactDetails> _items;
    ...
    public ObservableCollection<EmployeeContactDetails> AllItems { get; set; }
    public ObservableCollection<EmployeeContactDetails> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
   ...
   public int? SelectedBranchID {
    ...
    private void OnLoadData() {
      ...
      this.SetBusy();
      EmployeesDataManager.GetContactNumbers(DateTime.Today.ToMonth(), ListID != null ? ListID.Value.PackArray() : null, null, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeContactDetails[] result, Exception error) {
      if (error == null) {
        AllItems = new ObservableCollection<EmployeeContactDetails>(result.OrderBy(s => s.FullName));
        OnFilterData();
      } else
        Popup.AlertError(error);
    ...
    private void OnFilterData() {
      ...
      if (AllItems.Count() > 0 && lists.Count() > 0) {
        ...
        Items = new ObservableCollection<EmployeeContactDetails>(qry.ToArray());
      } else
        Items = new ObservableCollection<EmployeeContactDetails>();

      if (_needsSorting && this.ItemsGridView.SortDescriptors.Count() == 0) {
        this.ItemsGridView.SortBy(EmployeeContactDetails.Key_FullName);
      }
    ...
    private void ItemsGridView_RowActivated(object sender, Telerik.Windows.Controls.GridView.RowEventArgs e) {
      e.SelectParentRow<EmployeeContactDetails>();
      OnEditEmployee();
    }

    private void OnEditEmployee() {
      this.SetBusy();
      EmployeesDataManager.GetSingleEmployee(((EmployeeContactDetails)this.ItemsGridView.SelectedItem).EmployeeID, DateTime.Today, false, OnGetEmployee);
    }
    ...
    private void OnModifyWindowClosed(EmployeeModifyWindow obj) {
      ...
      this.SetBusy();
      EmployeesDataManager.GetContactNumbers(DateTime.Today.ToMonth(), ListID != null ? ListID.Value.PackArray() : null, new int[] { obj.Item.EmployeeID }, 
        OnSingleItemLoaded);
    }

    private void OnSingleItemLoaded(EmployeeContactDetails[] result, Exception error) {
