...
namespace Csc.IntelliSchool.Modules.HRModule.Views {
  public partial class BonusesPage : Csc.Wpf.PageBase {
    #region Fields
    ...
    private EmployeeBonusType[] _types;
    ...
    public ObservableCollection<EmployeeBonus> AllItems { get; set; }
    ...
    public EmployeeBonusType[] Types { get { return _types; } set { if (_types != value) { _types = value; OnPropertyChanged(() => Types); } } }
    ...
    public int[] SelectedTypeIDs {
      get {
        return this.TypeComboBox.GetSelectedItemIDs<EmployeeBonusType>(a => a.TypeID);
      }
      set {
        this.TypeComboBox.SetSelectedItemIDs<EmployeeBonusType>(value ?? new int[] { }, a => a.TypeID);
      }
    }
    ...
    private void OnGetBonusTypes() {
      this.SetBusy();
      EmployeesDataManager.GetBonusTypes(false, OnGetTypes);
    }
    ...
    private void OnGetTypes(EmployeeBonusType[] result, Exception error) {
      if (error == null) {
    ...
    private void OnLoadData(bool reload  = false) {
      ...
      if (SelectedTypeIDs.Count() == 0) {
        OnDataLoaded(new EmployeeBonus[] { }, null);
        return;
      }

      this.SetBusy();
      EmployeesDataManager.GetBonuses(SelectedTypeIDs, 
    ...
    private void OnDataLoaded(EmployeeBonus[] result, Exception error) {
      if (error == null) {
        ...
        AllItems = new ObservableCollection<EmployeeBonus>(result);
        OnFilterData();
      } else
        Popup.AlertError(error);
    ...
    private void OnFilterData(object itm  = null) {
      ...
      if (AllItems.Count() > 0 && lists.Count() > 0) {
        ...
        Items = new ObservableCollection<EmployeeBonus>(qry.ToArray());
      } else
    ...
    private void OnDeleteItem() {
      var itm = (EmployeeBonus)this.ItemsGridView.SelectedItem;
      Popup.ConfirmDelete(null, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteBonus (itm, OnDeleted);
      });
    }
    ...
    private void OnEmployeeSelected(EmployeeSelectionWindow obj) {
      ...
      BonusModifyWindow wnd = new BonusModifyWindow(obj.SelectedEmployee);
      this.DisplayModal<BonusModifyWindow>(wnd, OnModifyWindowClosed);
    }

    private void OnModifyWindowClosed(BonusModifyWindow obj) {
    ...
    private void OnEditItem() {
      var itm = (EmployeeBonus)this.ItemsGridView.SelectedItem;
      BonusModifyWindow wnd = new BonusModifyWindow(itm.Employee, itm);
      this.DisplayModal<BonusModifyWindow>(wnd, OnModifyWindowClosed);
    }
    #endregion
