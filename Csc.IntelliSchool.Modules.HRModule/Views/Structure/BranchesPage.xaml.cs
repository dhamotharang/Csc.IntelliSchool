...
namespace Csc.IntelliSchool.Modules.HRModule.Views.Structure {
  public partial class BranchesPage : Csc.Wpf.PageBase {
    #region Fields
    private ObservableCollection<EmployeeBranch> _items;
    #endregion


    #region Properties
    public ObservableCollection<EmployeeBranch> Items { get { return _items; } set { _items = value; OnPropertyChanged(() => Items); } }
    #endregion
    ...
    private void OnLoadData() {
      this.SetBusy();
      EmployeesDataManager.GetBranches(true, OnDataLoaded);
    }
    private void OnDataLoaded(EmployeeBranch[] result, Exception error) {
      if (error == null)
        Items = new ObservableCollection<EmployeeBranch>(result);
      else
    ...
    private void OnEditItem() {
      BranchModifyWindow wnd = new BranchModifyWindow((EmployeeBranch)this.ItemsGridView.SelectedItem);
    ...
    private void OnDeleteItem() {
      Popup.ConfirmDelete(null, () => {
        this.SetBusy();
        EmployeesDataManager.DeleteBranch((EmployeeBranch)this.ItemsGridView.SelectedItem, OnDeleted);
      });
    }
    ...
    private void OnNewItem() {
      BranchModifyWindow wnd = new BranchModifyWindow();
      wnd.Closed += ModifyWindow_Closed;
      this.DisplayModal(wnd);
    }
    ...
    void ModifyWindow_Closed(object sender, Telerik.Windows.Controls.WindowClosedEventArgs e) {
      ...
      var wnd = ((BranchModifyWindow)sender);
