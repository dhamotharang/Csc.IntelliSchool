...
namespace Csc.IntelliSchool.Modules.HRModule.Reports {
  public partial class MedicalListReport : Telerik.Reporting.Report {
    public MedicalListReport() {
      InitializeComponent();
    }
    ...
    private void BindFinancial(object sender, Employee[] items) {
      ...
      values.Add(new KeyValue<string, int?>());
      values.Add(new KeyValue<string, int?>("Salary Deductions", subItems.Sum(s=>s.Salary.Medical)));
